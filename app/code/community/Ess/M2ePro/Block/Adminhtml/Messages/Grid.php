...
class Ess_M2ePro_Block_Adminhtml_Messages_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    {
        $collection = Mage::getModel('M2ePro/Messages')->getCollection();
        ...
        $collection->addExpressionFieldToSelect('have_responses', 'if(`main_table`.`message_responses` = "", 0, 1)', 'message_responses');
    ...
    {
        $url = Mage::helper('M2ePro/Ebay')->getEbayMemberUrl($value,$row->getData('account_mode'));
    ...
    public function callbackColumnEbayItemId($value, $row, $column, $isExport)
    {
        if (is_null($row->getData('product_id'))) {
            ...
            $url = $this->getUrl('*/adminhtml_listings/goToEbay/', array('item_id' => $row->getData('product_id')));
            return '<a href="'.$url.'" target="_blank">'.$value.'</a>';
        }
    }
    ...
    public function callbackColumnMessage($value, $row, $column, $isExport)
    {
        ...
        if (strlen($messageText) > 135) {
            ...
            $messageText .= ' <a href="javascript:void(0);" onclick="MessagesHandlersObj.showMessageText(\''.$row->getId().'\')">'.Mage::helper('M2ePro')->__('Read More').'</a>';
        }
    ...
    public function callbackColumnResponses($value, $row, $column, $isExport)
    {
        ...
        foreach ($responses as $key => $response) {
            ...
            if (strlen($response) > 60) {
                ...
                $responseHtml .= ' <a href="javascript:void(0);" onclick="MessagesHandlersObj.showMessageResponse(\''.$row->getId().'\',
                                                                                                                  \''.$key.'\')">'.Mage::helper('M2ePro')->__('Read More').'</a>';
            } else {
        ...
        $buttonBlock = $this->getLayout()
                            ...
                            ->setData( array(
                                ...
                                'onclick' => 'MessagesHandlersObj.openMessage(this,
                                                                              \''.$row->getData('id').'\',
    ...
    {
        return $this->getUrl('*/*/gridMessages', array('_current'=>true));
    }
