...
{
    const CACHE_IDENTIFIER = 'm2epro_extension';
    ...
    {
        $cacheKey = self::CACHE_IDENTIFIER.'_'.$key;
        $value = Mage::app()->getCache()->load($cacheKey);
        $value !== false && $value = unserialize($value);
        return $value;
    }

    public function setCacheValue($key, $value, $lifeTime = NULL)
    {
        if (is_null($lifeTime) || (int)$lifeTime <= 0) {
            $lifeTime = 3600*24*365*5;
        }

        $cacheKey = self::CACHE_IDENTIFIER.'_'.$key;
        $tags = array(self::CACHE_IDENTIFIER.'_tag');

        Mage::app()->getCache()->save(serialize($value), $cacheKey, $tags, (int)$lifeTime);
    }
