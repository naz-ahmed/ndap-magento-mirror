...
class Ess_M2ePro_Model_ListingsProducts extends Mage_Core_Model_Abstract
{
    ...
    {
        $src = $this->getListingTemplate()->getLocalShippingCashOnDeliverySource();

        if ($src['mode'] == Ess_M2ePro_Model_ListingsTemplates::CASH_ON_DELIVERY_COST_MODE_CUSTOM_VALUE) {
            return (float)$src['value'];
        }

        if ($src['mode'] == Ess_M2ePro_Model_ListingsTemplates::CASH_ON_DELIVERY_COST_MODE_CUSTOM_ATTRIBUTE) {
