...
class Ess_M2ePro_Model_Orders_Order extends Mage_Core_Model_Abstract
{
    ...
    public function getPreparedShippingAddress()
    {
        if (is_null($this->_preparedShippingAddress)) {
            ...
            if (empty($addressInformation['region_id'])) {
                $addressInformation['region_id'] = 1;
            }
            if (empty($addressInformation['region'])) {
                $addressInformation['region'] = '';
            }

            $region = Mage::getModel('directory/region')->getCollection()
                                                        ->addFieldToFilter('country_id', $addressInformation['country_id'])
                                                        ->addFieldToFilter('code', $addressInformation['region_id'])
                                                        ->getFirstItem();
            $addressInformation['region_id'] = $region->getData('region_id') ? $region->getData('region_id') : 1;

            if (empty($addressInformation['postcode'])) {
                $addressInformation['postcode'] = '0000';
            }
            if (empty($addressInformation['telephone'])) {
            ...
            if (empty($addressInformation['firstname'])) {
                $addressInformation['firstname'] = 'Name';
            }
            if (empty($addressInformation['lastname'])) {
                $addressInformation['lastname'] = 'Surname';
            }
            if (empty($addressInformation['email']) || stripos($addressInformation['email'], 'invalid request') !== false) {
                $addressInformation['email'] = $this->getData('buyer_userid') . '@' . $this->getData('buyer_userid') . '.com';
            }
    ...
    /**
     * Return orders which contains at least one order item (item id and transaction id pair) from current order
     *
     * @return Varien_Data_Collection_Db|null
     */
    public function getRelatedOrdersCollection()
    {
        if (!$this->isCombined()) {
            return NULL;
        }
        ...
        foreach ($this->getItemsCollection()->getItems() as $orderItem) {
            $transactionOrderIds[] = '(item_id = ' . (float)$orderItem->getData('item_id') . ' AND transaction_id = ' . (float)$orderItem->getData('transaction_id') . ')';
        }
        $expression = implode(' OR ', $transactionOrderIds);

        $collection = Mage::getModel('M2ePro/Orders_Order')->getCollection();
        $collection->getSelect()
                   ->distinct(true)
                   ->join(array('mooi'=>Mage::getResourceModel('M2ePro/Orders_OrderItem')->getMainTable()),
                          '(`mooi`.`ebay_order_id` = `main_table`.`id`)',
                          array())
                   ->where($expression)
    ...
    public function canCreateMagentoOrder($isFrontend = false)
    {
        ...
        if ($this->isCombined() && $this->getAccount()->isOrdersCombinedDisabled()) {
            // Parser hack -> Mage::helper('M2ePro')->__('Magento Order was not created. Reason: "Prioritize Combined Orders" option is set to "No" in eBay account settings.');
            $this->addWarningLogMessage('Magento Order was not created. Reason: "Prioritize Combined Orders" option is set to "No" in eBay account settings.');
            return false;
        }
