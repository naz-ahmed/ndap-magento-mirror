...
class Ess_M2ePro_Model_Orders_Ebay_Order extends Mage_Core_Model_Abstract
{
    ...
    protected $_updateCheckoutMessage = false;
    ...
    protected function initializeData(array $data = array())
    {
        ...
        $this->_data['final_value_fee'] = $data['final_value_fee'];
    ...
    protected function createOrder()
    {
        ...
        if ($this->isNew()) {
            ...
            if ($order->hasMagentoOrder()) {
                ...
                // ------------------
                if ($order->getShippingAddress() != $this->getData('shipping_address')) {
                ...
                // Check checkout message
                // ------------------
                if ($order->getData('checkout_message') != $this->getData('checkout_message') &&
                    $this->getData('checkout_message') != '') {
                    $this->_updateCheckoutMessage = true;
                }
                // ------------------
            }
    ...
    {
        if ($this->_updateShippingAddress || $this->_updatePaymentData || $this->_updateCheckoutMessage) {
            /** @var $magentoOrder Ess_M2ePro_Model_Orders_Magento_Order */
            $magentoOrder = Mage::getModel('M2ePro/Orders_Magento_Order');
            $magentoOrder->setOrder($this->getOrder());

            $this->_updateShippingAddress && $magentoOrder->updateAddress();
            $this->_updatePaymentData     && $magentoOrder->updatePaymentData();
            $this->_updateCheckoutMessage && $magentoOrder->updateCheckoutMessage();
        }
    }
