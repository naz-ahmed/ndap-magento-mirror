...
class Ess_M2ePro_Adminhtml_ListingsController extends Ess_M2ePro_Controller_Adminhtml_MainController
{
    ...
    protected function _initAction()
    {
        ...
        $this->getLayout()->getBlock('head')
             ...
             ->addJs('M2ePro/Components/AutoComplete.js')
             ->addCss('M2ePro/css/Components/AutoComplete.css');
    ...
    public function addStepOne()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($post['synchronization_start_type'] != 3) {
            ...
            if ($post['synchronization_stop_type'] != 2) {
            ...
            $_SESSION['M2ePro_data'] = array(
                ...
                'selling_format_template_title' => Mage::getModel('M2ePro/SellingFormatTemplates')
                                                   ->load((int)$post['selling_format_template_id'])
                                                   ->getTitle(),
                'listing_template_id'     => $post['listing_template_id'],
                'listing_template_title'  => Mage::getModel('M2ePro/ListingsTemplates')
                                                   ->load((int)$post['listing_template_id'])
                                                   ->getTitle(),
                'description_template_id'    => $post['description_template_id'],
                'description_template_title' => Mage::getModel('M2ePro/DescriptionsTemplates')
                                                   ->load((int)$post['description_template_id'])
                                                   ->getTitle(),
                'synchronization_template_id'    => $post['synchronization_template_id'],
                'synchronization_template_title' => Mage::getModel('M2ePro/SynchronizationsTemplates')
                                                   ->load((int)$post['synchronization_template_id'])
                                                   ->getTitle(),
                ...
                'synchronization_stop_date' => $synchronization_stop_date,
    ...
    public function editAction()
    {
        ...
        $additionalData = array(
            'selling_format_template_title'  => Mage::getModel('M2ePro/SellingFormatTemplates')
                                               ->load($model->getData('selling_format_template_id'))
                                               ->getTitle(),
            'listing_template_title'         => Mage::getModel('M2ePro/ListingsTemplates')
                                               ->load($model->getData('listing_template_id'))
                                               ->getTitle(),
            'description_template_title'     => Mage::getModel('M2ePro/DescriptionsTemplates')
                                               ->load($model->getData('description_template_id'))
                                               ->getTitle(),
            'synchronization_template_title' => Mage::getModel('M2ePro/SynchronizationsTemplates')
                                                ->load($model->getData('synchronization_template_id'))
                                                ->getTitle(),
        );
        Mage::register('M2ePro_data', array_merge($model->getData(), $additionalData));
    ...
    public function productsAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            foreach ($selectedProductsIds as $productId) {
                ...
                $ids[] = $productInstance->getId();
            }
            //---------------
            if ($this->getRequest()->getParam('do_list')) {
                $_SESSION['products_ids_for_list'] = implode(',',$ids);
            }
    ...
    public function goToDescriptionTemplateAction()
    {
        ...
        $this->_redirect('*/adminhtml_descriptionTemplates/edit', array('id' => $model->getData('description_template_id'),'back'=>Mage::helper('M2ePro')->getBackUrlParam('list')));
    }
